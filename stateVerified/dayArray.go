	...
	for numDay, day := range sets.DaySets {
		...
		for numLine, line := range day.Lines {
			...
			if line.Hour == 24 && line.Min != 0 {
				result.SumResult = append(result.SumResult, fmt.Sprintf("Не верно указано значение карты № (%v) стр. № (%v): значение минуты = %v должно быть 0", numDay+1, numLine+1, line.Min))
				result.Err = errors.New("detected")
			}
			...
			if line.PKNom != 0 {
				...
				if flagZero {
					if line.Hour != 0 || line.Min != 0 || line.PKNom != 0 {
						result.SumResult = append(result.SumResult, fmt.Sprintf("Не верно заполнено значение карты № (%v) стр. № (%v): значение времени должно быть 00:00 и #ПК 0", numDay+1, numLine+1))
						result.Err = errors.New("detected")
					}
				}
			} else {
